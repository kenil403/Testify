...
import { 
  getChemicalPaperSet1,
  getChemicalPaperSet2,
  getChemicalPaperSet3,
  getChemicalPaperSet4,
  getChemicalPaperSet5
} from '../data/course_chemical';

const ChemicalSolutionPage = ({ navigate, currentUser, testState }) => {
  // Determine the selected paper number for Chemical Engineering
  let paperNumber = 1;
  if (testState?.selectedPaperNumber) {
    paperNumber = testState.selectedPaperNumber;
  } else if (currentUser?.email) {
    const stored = localStorage.getItem(`user_${currentUser.email}_current_chemical_paper`);
    if (stored) {
  ...
  const paper = paperNumber === 2 ? getChemicalPaperSet2() :
                paperNumber === 3 ? getChemicalPaperSet3() :
                paperNumber === 4 ? getChemicalPaperSet4() :
                paperNumber === 5 ? getChemicalPaperSet5() :
                getChemicalPaperSet1();
  ...
  return (
    <div className="container mx-auto min-h-screen flex flex-col">
      ...
      <div className="text-center sticky top-0 bg-white z-10 pt-4 pb-4 border-b mb-4">
        <h1 className="text-5xl font-extrabold text-slate-800">⚗️ Chemical Engineering Solutions</h1>
        <p className="text-lg text-slate-500 mt-4 max-w-3xl mx-auto">Complete question paper with answers and detailed explanations. Review all {bank.length} questions with solutions.</p>
      </div>
...
export default ChemicalSolutionPage;
