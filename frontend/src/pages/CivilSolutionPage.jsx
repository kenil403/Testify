...
import { 
  ...
  getCivilPaperSet4,
  getCivilPaperSet5
} from '../data/course_civil';

const CivilSolutionPage = ({ navigate, currentUser, testState }) => {
  let paperNumber = 1;

  if (testState?.selectedPaperNumber) {
    paperNumber = testState.selectedPaperNumber;
  } else if (currentUser?.email) {
    const stored = localStorage.getItem(`user_${currentUser.email}_current_civil_paper`);
    if (stored) {
  ...
  const paper = paperNumber === 2 ? getCivilPaperSet2() :
                paperNumber === 3 ? getCivilPaperSet3() :
                paperNumber === 4 ? getCivilPaperSet4() :
                paperNumber === 5 ? getCivilPaperSet5() :
                getCivilPaperSet1();
...
export default CivilSolutionPage;
