...
import { getMechanicalPaperSet1, getMechanicalPaperSet2, getMechanicalPaperSet3, getMechanicalPaperSet4, getMechanicalPaperSet5 } from '../data/course_mechanical';

const MechanicalSolutionPage = ({ navigate, currentUser, testState }) => {
    ...
    if (testState?.selectedPaperNumber) {
        ...
        const storedPaper = localStorage.getItem(`user_${currentUser.email}_current_mechanical_paper`);
        if (storedPaper) {
            paperNumber = parseInt(storedPaper);
        }
    }
    
    const paper = paperNumber === 2 ? getMechanicalPaperSet2() : 
                  paperNumber === 3 ? getMechanicalPaperSet3() : 
                  paperNumber === 4 ? getMechanicalPaperSet4() :
                  paperNumber === 5 ? getMechanicalPaperSet5() :
                  getMechanicalPaperSet1();
    ...
    return (
        <div className="container mx-auto min-h-screen flex flex-col">
            ...
            {/* Sticky header: stays visible while only MCQs scroll */}
            <div className="text-center sticky top-0 bg-white z-10 pt-4 pb-4 border-b mb-4">
                <h1 className="text-5xl font-extrabold text-slate-800">
                    🔧 Mechanical Engineering Solutions {paperNumber === 2 ? '- Paper 2' : paperNumber === 3 ? '- Paper 3' : paperNumber === 4 ? '- Paper 4' : paperNumber === 5 ? '- Paper 5' : ''}
                </h1>
...
export default MechanicalSolutionPage;
