...
import { 
  ...
  getElectronicsPaperSet2,
  getElectronicsPaperSet3,
  getElectronicsPaperSet4,
  getElectronicsPaperSet5
} from '../data/course_electronics';

const ElectronicsSolutionPage = ({ navigate, currentUser, testState }) => {
  // Determine the selected paper number for Electronics & Communication
  let paperNumber = 1;
  if (testState?.selectedPaperNumber) {
    paperNumber = testState.selectedPaperNumber;
  } else if (currentUser?.email) {
    const stored = localStorage.getItem(`user_${currentUser.email}_current_electronics_paper`);
    if (stored) {
  ...
  const paper = paperNumber === 2 ? getElectronicsPaperSet2() :
                paperNumber === 3 ? getElectronicsPaperSet3() :
                paperNumber === 4 ? getElectronicsPaperSet4() :
                paperNumber === 5 ? getElectronicsPaperSet5() :
                getElectronicsPaperSet1();
  ...
  return (
    <div className="container mx-auto">
      ...
      <div className="text-center mb-12">
        <h1 className="text-5xl font-extrabold text-slate-800">📡 Electronics & Communication Solutions</h1>
              ...
              <div className="space-y-3 mb-6">
                {mcq.options.map((option, optIndex) => {
                  ...
                  return (
                    <div
                      key={option}
                      className={`p-3 rounded-lg border-2 ${isCorrect ? 'bg-green-100 border-green-300 text-green-800' : 'bg-white border-slate-200 text-slate-700'}`}
                    >
...
export default ElectronicsSolutionPage;
