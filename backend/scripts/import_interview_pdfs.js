...
async function importInterviewPdfs() {
    try {
        await mongoose.connect(process.env.MONGODB_URI || process.env.MONGODB);
        console.log('Connected to MongoDB');

        const basePath = path.join(__dirname, '..', '..', 'project-papers', 'Interview');
        
        const subsections = [
            { name: 'HR Interview Questions', files: ['HR Interview Questions1.pdf'] },
            { name: 'Resume Building', files: ['Resume Building1.pdf'] },
            { name: 'Technical Questions', files: ['Technical Questions1.pdf'] }
        ];
